...
my $GFF = shift;
die "
Please provide a GFF3 file with mRNA and exon features

./getTranscriptInfo_fromGFF.pl GFF3-file
...
while (my $line = <GFF>){
  ...
  if ($type eq 'mRNA'  or $type eq 'nontranslating_CDS'){
    my ($id) = $nine =~ /ID=([^;]+)/;
    my ($name) = $nine =~ /Name=([^;]+)/;
    if (!defined $name){
...
foreach my $ref (sort keys %genes){
  foreach my $gene (sort keys %{$genes{$ref}}){
    ...
    my $strand =  $genes{$ref}{$gene}{strand};
    ...
    print OUTEXONS "$name\t$ref\t$strand\t$start,$end\t",join(';',@exons),"\n";
  }
} 
