...
my $db_dir = "dbs/$organism/genomic";    ## genome file
my $exon_file = "dbs/$organism/cDNA/transcript_exons.txt"
my %exons;
createExonInfo();
      ...
      foreach my $strand (%hits) {
        ...
        { 
            ...
            @{ $hits{$other_strand}{$p2} } )
          {
            ...
            my $product_start = $sorted_starts[0];
            my $product_end =  $sorted_ends[0];
            my $product_size = $product_end - $product_start  + 1;
            my $exon_info_sum=0;
            my @exon_info;
            if ($type eq 'cDNA'){
              #my ($t_id,$loc,$tie_array_id) = split /\|/ ,$tName ;
              #$tName = $t_id;
              my @exon_starts = sort {$a <=> $b} %{$exons{$tName}} ;
              my %keepers;
              for (my $i = 0 ; $i<@exon_starts ; $i++){
                $exon_start = $exon_starts[$i];
        
                if ($product_start < $exon_start and !exists $keepers{first_exon}){
                  $keepers{first_exon}=$exon_starts[$i -1];
                }elsif($product_start == $exon_start and !exists $keeper{first_exon}){
                  $keepers{first_exon}=$exon_starts[$i];
                }elsif($product_end ){

                }
if ( $product_start < $exon_start){
...
sub createExonInfo {
  open EXONFILE , "$exon_file" or die $!;
  while (my $line = <EXONFILE>){
    chomp $line;
    my ($gene_id, @ranges) = split /\t/ , $line;
    my ($gene_name) = split /\|/ , $gene_id;
    my $exon_num = 0;
    foreach my $range (@ranges){
      $exon_num++;
      my ($ref,$s,$e) = $range =~ /(.+):(\d+)\.\.(\d+)/;
      #$exons{$ref}{$s}{$gene_name}{$exon_num}=$e;
      $exons{$ref}{$s}{$e}{$gene_name}=$exon_num;
    }
  }
}
